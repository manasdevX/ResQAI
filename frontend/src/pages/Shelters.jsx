...
const TYPE_LABELS = {
  ...
  community_hall:      { label: 'Community Hall',      icon: '🏛️', color: 'text-purple-400 bg-purple-500/10 border-purple-500/30' },
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap gap-1 mt-3">
        ...
        {AMENITY_ICONS.filter(a => shelter.amenities?.[a.key]).length > 5 && (
          <span className="text-[10px] text-zinc-500 px-1.5 py-0.5">+{AMENITY_ICONS.filter(a => shelter.amenities?.[a.key]).length - 5} more</span>
        )}
      </div>
...
const Shelters = () => {
  ...
  const hospitalCount = shelters.filter(s => s.type === 'hospital').length;
  const totalSpots    = shelters.reduce((acc, s) => acc + Math.max(0, (s.totalCapacity || 0) - (s.currentOccupancy || 0)), 0);
      ...
      <header className="flex items-center justify-between px-6 py-3 bg-zinc-900 border-b border-zinc-800 shrink-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-400 leading-none mt-0.5">Shelter & Hospital Finder</p>
          </div>
        </div>
        <div className="hidden md:flex items-center gap-3 text-sm">
          <div className="flex items-center gap-1.5 px-3 py-1 bg-zinc-800 rounded-full">
            <span className="w-2 h-2 bg-green-400 rounded-full" />
            <span className="text-zinc-300">{activeCount} Active Shelters</span>
          </div>
          <div className="flex items-center gap-1.5 px-3 py-1 bg-red-950/60 border border-red-900/40 rounded-full">
            <span className="text-red-400">🏥 {hospitalCount} Hospitals</span>
          </div>
          <div className="flex items-center gap-1.5 px-3 py-1 bg-zinc-800 rounded-full">
            <Users className="w-3.5 h-3.5 text-zinc-400" />
            <span className="text-zinc-300">{totalSpots.toLocaleString()} spots available</span>
          </div>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-80 xl:w-96 bg-zinc-900 border-r border-zinc-800 flex flex-col overflow-hidden shrink-0">
          ...
          <div className="px-4 pt-4 pb-3 border-b border-zinc-800 space-y-3 shrink-0">
              ...
              {!searchNearby ? (
                ...
                <div className="flex-1 flex items-center justify-between py-2 px-3 bg-blue-950/40 border border-blue-800/40 rounded-lg">
                  ...
                  <button onClick={handleClearLocation}>
                    <X className="w-4 h-4 text-blue-400 hover:text-blue-200" />
                  </button>
                </div>
              )}
            ...
            {/* Radius slider (when nearby) */}
            {searchNearby && (
              <div>
                <label className="text-xs text-zinc-400 mb-1 block">Search radius: <span className="text-white font-semibold">{radius} km</span></label>
          ...
          <div className="px-4 py-2 border-b border-zinc-800/60 shrink-0">
            <p className="text-xs text-zinc-500">
              {loading ? 'Loading…' : `${filtered.length} shelter${filtered.length !== 1 ? 's' : ''} found`}
              {searchNearby && userLocation && !loading && ` within ${radius} km`}
            </p>
          </div>
        ...
        <div className="flex-1 relative">
          ...
          <div className="absolute bottom-6 left-4 bg-zinc-900/90 backdrop-blur-sm border border-zinc-700 rounded-xl p-3 text-xs space-y-1.5 z-10">
            ...
            {[
              ...
              <div key={label} className="flex items-center gap-2">
                {/* Triangle matching the BACKWARD_CLOSED_ARROW map marker shape */}
                <span style={{
          ...
          <div className="absolute top-4 left-4 flex items-center gap-2 px-3 py-1.5 bg-zinc-900/90 backdrop-blur-sm rounded-full border border-zinc-700 text-xs font-medium z-10">
            ...
            {filtered.length} shelter{filtered.length !== 1 ? 's' : ''} on map
          </div>
        </div>
      </div>
    </div>
