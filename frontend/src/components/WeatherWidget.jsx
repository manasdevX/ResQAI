...
const WeatherWidget = ({ compact = false }) => {
  ...
  if (loading) {
    return (
      <div className={`flex items-center gap-2 ${compact ? 'px-3 py-2 bg-zinc-800/60 rounded-xl' : 'p-4 bg-zinc-900/80 border border-zinc-800/80 rounded-2xl'}`}>
    ...
    <div className="p-4 bg-zinc-900/80 border border-zinc-800/80 rounded-2xl">
      ...
      <div className="flex items-center gap-4 mt-3 pt-3 border-t border-zinc-800/80">
        ...
        <div className="flex items-center gap-1.5 text-xs text-zinc-400">
          ...
          <span>{weather.isDay ? 'Daytime' : 'Night-time'}</span>
        </div>
      </div>
